...
def create_matplotlib_image(data):
...
def get_auto_collections(base_path):
    ...
    regex_queries = [{"request_obj.directory": {"$regex": f"{path.relative_to(path_to_remove)}"}} for path in base_path.iterdir() if not path.stem.endswith("_dir")]
    auto_collections = request_collection.find({"$or": regex_queries, 
...
def get_sample_data_and_rasters(auto_collections):
    ...
    for standard_collection in tqdm.tqdm(auto_collections):
        ...
        rasters = list(request_collection.find({"request_obj.parentReqID": standard_collection['uid']}))
        sample_name = sample['name']
        sample.pop("_id", None)
        all_data[sample_name]['sample'] = sample
        all_data[sample_name]['rasters'] = defaultdict(dict)
        all_data[sample_name]['standard'] = {}
        for raster in rasters:
            raster.pop("_id", None)
            all_data[sample_name]['rasters'][standard_collection['uid']][raster['uid']] = raster

        standard_collection.pop("_id", None)
        all_data[sample_name]['standard'][standard_collection['uid']] = standard_collection
    return all_data
...
def get_raster_spot_count(req, include_files=False):
    client = pymongo.MongoClient(os.environ["MONGODB"])
    res_obj = client.analysisstore.analysis_header.find_one({"request": req['uid'], "result_type": "rasterResult"})
    if res_obj is None:
        return np.zeros((2,2))
    ...
    try:
        snake_array = create_snake_array(spot_counts, *determine_raster_shape(req["request_obj"]["rasterDef"]))
...
def get_spot_positions(req, indices, reso_table):
    ...
    beam_center = np.array([int(float(req["request_obj"]["xbeam"])), int(float(req["request_obj"]["ybeam"])), 0])
    ...
    for i, idx in enumerate(indices):
        row = (idx - 1) // req["request_obj"]["rasterDef"]["rowDefs"][0]["numsteps"]
        path = Path(req["request_obj"]["directory"]) / Path(f"dozor/row_{row}/{idx:05d}.spot")
        if path.exists():
            df = pd.read_csv(path, skiprows=3, delimiter='\s+', header=None).to_numpy()[:,1:4]
            diff = df - beam_center
            distances = np.sqrt(diff[:,0]**2 + diff[:,1]**2)
            radius = np.max(distances)
            circle = Circle((0, 0), radius, color='red', fill=False, linestyle="--")
            circle2 = Circle((0, 0), radius/2, color='red', fill=False, linestyle="--")
            ...
            axs[i].scatter(diff[:,0],diff[:,1],c=np.log(df[:,2]),cmap='inferno',marker='.')
...
def get_jpeg_path(raster_req, base_path):
    # base_path = 
    rel_data_dir = Path(raster_req['request_obj']['directory']).resolve().relative_to(Path(raster_req['request_obj']['basePath']).resolve())
...
def load_collection_data_from_disk(json_path):
